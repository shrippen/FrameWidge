...
- [x] **What the small dot in icon-only tray mode does**, for the record: it's the same temperature-graded status indicator added in Phase 1/2 (`ColorGrading.gradeIndicatorColor`) — green/amber/red by CPU temp, grey when the service is offline — so there's still an at-a-glance health signal even when the tray slot shows only the icon and no number. It's independent of the new combined mode above (which shows the actual number instead of just a color).
...
- [x] Tab bar icons are now genuinely monochrome. Two of the five (`Power`, `Battery`) were silently using Breeze's full-color status icon variants (`battery-charging`, `battery-100`) instead of the `-symbolic` monochrome ones; fixed by switching to `temperature-warm-symbolic` / `speedometer-symbolic` / `battery-100-symbolic` / `configure-symbolic` (all confirmed to exist in the installed Breeze theme before use). No good `-symbolic` icon existed for "power management" specifically, so the Power tab ships its own lightning-bolt SVG (`icons/tab-power.svg`) via a custom `contentItem` — `icon.color` was confirmed (by rendering a test window) to **not** recolor an arbitrary `icon.source` on `QQC2.TabButton`, only named theme icons auto-tint, so `Kirigami.Icon`'s proven `isMask` was used directly instead of relying on that.
