...
- [x] Polish the Sensors chart and Curve editor: `SensorsPage` now has a hover crosshair with a floating per-series value readout (nearest-sample lookup) and a proper empty state; `CurveEditor` shows a floating "`X°C → Y%`" readout while dragging or hovering a point, and line joins are rounded for smoother rendering. Tuned-for-light-and-dark palette work is still open (current sensor palette is a fixed hash-based hex list, not theme-derived).
...
- [x] Accessibility pass: `CurveEditor` now supports keyboard control — Tab/Shift+Tab cycles the selected point, arrow keys nudge it (Shift for a 5-unit step), Delete/Backspace removes it (down to the 2-point minimum), with a visible focus ring and `Accessible.name`/`description` set. Screen-reader labeling for the rest of the icon-only controls (tab icons, buttons) is still open.
